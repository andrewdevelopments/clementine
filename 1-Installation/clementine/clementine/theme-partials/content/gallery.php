...
$galleryFormat = get_post_meta( $post->ID, '_format_gallery_type', true);
...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<?php if( !is_single() ) : ?>
		<header class="entry-header <?php if( !has_post_thumbnail() || $gallery ) : echo 'no-thumbnail'; endif;?> <?php if( $fullheight_article == 'on' || has_post_thumbnail() && $fullheight_article == '' ) : echo 'fullheight-thumbnail'; endif; ?> <?php if( ( _clementine_get_option_('adD_custom_excerpt_length') || strpos($post->post_content, '<!--more-->') ) && ( $fullheight_article == 'off' || $fullheight_article == '' ) ) :  echo 'has-content'; endif; ?>">
			<?php if( !$gallery && has_post_thumbnail() && $fullheight_article == 'off' ) : ?>
				<!-- Post Media -->
				...
				<!-- End Post Media -->
			<?php elseif( !$gallery && has_post_thumbnail() && $fullheight_article == 'on' ) : ?>
				<!-- Full height media -->
				<div class="post-media">
					<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
			...
			<?php elseif( !$gallery && has_post_thumbnail() && $fullheight_article == '' ) : ?>
				<!-- Normal media -->
				<div class="post-media">
					<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
			...
			<?php elseif( $gallery ) : ?>
				<div class="post-gallery">
					<?php foreach($gallery as $image) : ?>
						...
						<?php $getImage = wp_get_attachment_image_src( $image, 'full-thumb' ); ?>
						<?php $the_caption = get_post_meta($image, '_wp_attachment_image_alt', true ); ?>
					
						<a href="<?php the_permalink(); ?>">
							<img src="<?php echo $getImage[0]; ?>" alt="<?php echo esc_html( $the_caption ); ?>" />
						</a>
		...
		<?php if( $fullheight_article == 'off' || $fullheight_article == '' ) : ?>
			<!-- Excerpt -->
			<div class="entry-content">
				<?php if( _clementine_get_option_('adD_custom_excerpt_length') ) : ?>
					<?php echo clementine_excerpt( _clementine_get_option_('adD_custom_excerpt_length') ); ?>
				<?php else :
